...
declare
   ...
   l_ENTITY_NM  VARCHAR2(32) := 'acct_26';
   l_SHEET_NM   VARCHAR2(32) := 'Д/п не равен 100(2)';    
   ...
   l_DESCR      clob := Q'##';
   l_SQL_HEADER clob := Q'##';
   l_SQL_QUERY  clob := Q'#select * from ${acct_26}#';
   l_SQL_DATA   clob := Q'#
  select * from (
select distinct
       tmp.cis_division
      ...
      , tmp.sa_dproc "Сумма значений ДПИ"
      , sa.sa_id  "ID РДО"
      , ( 
           select descr

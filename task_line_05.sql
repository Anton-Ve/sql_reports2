...
declare
   ...
   l_ENTITY_NM  VARCHAR2(32) := 'acct_05';
   l_SHEET_NM   VARCHAR2(32) := 'Кол-во прописанных';                      -- Кол-во прописанных нецифровое знач.
   ...
   l_DESCR      clob := Q'#5#';
   l_SQL_HEADER clob := Q'##';
   l_SQL_QUERY  clob := Q'#select * from ${acct_05}#';
   l_SQL_DATA   clob := 
Q'#select 
  distinct pc.prem_id
, pc.srch_char_val as KOL_PROP
, p.cis_division
from ci_prem_char pc
left join ci_prem p on p.prem_id=pc.prem_id
where pc.char_type_cd='KOL-PROP'

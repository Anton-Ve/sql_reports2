...
declare
   ...
   l_ENTITY_NM  VARCHAR2(32) := 'acct_14';
   ...
   l_DESCR      clob := Q'#14#';
   l_SQL_HEADER clob := Q'##';
   l_SQL_QUERY  clob := Q'#select * from ${acct_14}#';
   l_SQL_DATA   clob := 
Q'#select * from ${acct_13} vz
   where not exists ( select 1 from ci_stm_cnst s where s.descr50=vz.descr50 and s.eff_status!='A')
   #';
   l_index number;
begin

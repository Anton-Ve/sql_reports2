...
declare
   ...
   l_ENTITY_NM  VARCHAR2(32) := 'acct_16';
   l_SHEET_NM   VARCHAR2(32) := '1 ЛС на нескольких ССВ';    
   ...
   l_DESCR      clob := Q'#16#';
   l_SQL_HEADER clob := Q'##';
   l_SQL_QUERY  clob := Q'#select * from ${acct_16}#';
   l_SQL_DATA   clob := 
Q'#
select 
  vz.cis_division
 ...
 ,vz.eff_status
 ,vz.descr50
 ,count(cd.acct_id) as count
 from ${acct_15} vz
 left join ci_stm_cnst_dtl cd on cd.stm_cnst_id=vz.stm_cnst_id
 where vz.cis_division in (select /*+ cardinality 1*/ CAST(Trim(r.VL) as char(5)) cis_division from TABLE( VZ_REPORT_API.split('${PARAMETER_01}')) r) 
 group by 
  vz.cis_division
 ,vz.stm_cnst_id
 ,vz.per_id

COLUMN TASK_SEQ NEW_V TASK_SEQ_CURRENT
select (&TASK_SEQ_CURRENT.+1) TASK_SEQ from dual;

PROMPT VZ_TASK_BASE_LINE.&TASK_CAT..&TASK_CD..&TASK_SEQ_CURRENT..merge - start
declare
   l_seq        number := &TASK_SEQ_CURRENT;
   l_task_cd    VARCHAR2(16) := '&TASK_CD.';
   l_cat_cd     VARCHAR2(16) := '&TASK_CAT.';
   l_ENTITY_NM  VARCHAR2(32) := 'acct_12';
   l_SHEET_NM   VARCHAR2(32) := 'Неверно закрытые РДО ПВК';    
   l_SHEET_COL  VARCHAR2(32) := '';
   l_SQL_OP     VARCHAR2(7)  := 'CREATE';
   l_ENABLE     number := 1;
   l_EXPORT	    number := 1;
   l_LINE_BUILD number := 1;
   l_DESCR      clob := Q'#12#';
   l_SQL_HEADER clob := Q'##';
   l_SQL_QUERY  clob := Q'#select * from ${acct_12}#';
   l_SQL_DATA   clob := 
Q'#
with t_sa as (
  select sa.sa_id
       , sa.acct_id
       , sa.sa_type_cd
       , sa.sa_status_flg
       , sa.start_dt
       , sa.end_dt
    from ci_sa           sa
       , ci_sa_type_char stc
   where 1 = 1
     and stc.sa_type_cd = sa.sa_type_cd
     and stc.cis_division = sa.cis_division
     and stc.char_type_cd = 'TIP-RDO'
     and stc.char_val = 'USL'
     and stc.sa_type_cd not like '%NORM%'
),
t_norm as (
  select sa.sa_id
       , sa.acct_id
       , sa.sa_type_cd
       , sa.sa_status_flg
       , sa.start_dt
       , sa.end_dt
    from ci_sa           sa
       , ci_sa_type_char stc
   where 1 = 1
     and stc.sa_type_cd = sa.sa_type_cd
     and stc.cis_division = sa.cis_division
     and stc.char_type_cd = 'TIP-RDO'
     and stc.char_val = 'USL'
     and stc.sa_type_cd like '%NORM%'
)
select a.cis_division     "ПАУ"
     , (
           select descr
             from ci_acct_char  o
                , ci_char_val_l cvl
            where 1 = 1
              and o.acct_id = a.acct_id
              and o.char_type_cd = 'OTDELEN'
              and o.effdt = (
                  select max(effdt)
                    from ci_acct_char a1
                   where 1 = 1
                     and a1.acct_id = o.acct_id
                     and a1.char_type_cd = o.char_type_cd
              )
              and cvl.char_type_cd = o.char_type_cd
              and cvl.char_val = o.char_val
              and cvl.language_cd = 'RUS'
     ) "Отделение"
     , (
           select descr
             from ci_acct_char  o
                , ci_char_val_l cvl
            where 1 = 1
              and o.acct_id = a.acct_id
              and o.char_type_cd = 'U4ASTOK'
              and o.effdt = (
                  select max(effdt)
                    from ci_acct_char a1
                   where 1 = 1
                     and a1.acct_id = o.acct_id
                     and a1.char_type_cd = o.char_type_cd
              )
              and cvl.char_type_cd = o.char_type_cd
              and cvl.char_val = o.char_val
              and cvl.language_cd = 'RUS'
     ) "Участок"
     , (
           select descr
             from ci_acct_char  o
                , ci_char_val_l cvl
            where 1 = 1
              and o.acct_id = a.acct_id
              and o.char_type_cd = 'SVC-TYPE'
              and o.effdt = (
                  select max(effdt)
                    from ci_acct_char a1
                   where 1 = 1
                     and a1.acct_id = o.acct_id
                     and a1.char_type_cd = o.char_type_cd
              )
              and cvl.char_type_cd = o.char_type_cd
              and cvl.char_val = o.char_val
              and cvl.language_cd = 'RUS'
     ) "Вид услуги"
     , sa.sa_id   "ИД РДО"
     , (
          select h.rs_cd
            from ci_sa_rs_hist h
           where 1 = 1
             and h.sa_id = sa.sa_id
             and h.effdt = (
                 select max(effdt)
                   from ci_sa_rs_hist h1
                  where 1 = 1
                    and h1.sa_id = h.sa_id
             )
     ) "План расчета"
     , sa.sa_status_flg   "Статус РДО"
     , sap.stop_dttm      "Дата-время завер-я РДО/ТУ"
     , sa.end_dt          "Дата закрытия РДО"
     , nrm.sa_id          "ИД РДО ПВК"
     , nrm.sa_status_flg  "Статус РДО ПВК"
     , sapn.stop_dttm     "Дата-время завер-я РДО/ТУ ПВК"
     , nrm.end_dt         "Дата закрытия РДО ПВК"
  from ci_acct  a
     , t_sa     sa
     , t_norm   nrm
     , ci_sa_sp sap
     , ci_sa_sp sapn
 where 1 = 1
   and sa.acct_id = a.acct_id
   and nrm.acct_id = a.acct_id
   and sap.sa_id = sa.sa_id
   and sapn.sa_id = nrm.sa_id
   and (
        ( nrm.sa_status_flg = '30' and sa.sa_status_flg = '20' ) or
        ( nrm.sa_status_flg = '20' and sa.sa_status_flg not in ('20', '70') ) or
        ( nrm.sa_status_flg = '30' and sa.sa_status_flg in ( '20', '30', '60' )
        and sap.stop_dttm = sapn.stop_dttm )
   )
   and exists (
       select 1
         from ci_sa_char sc
        where 1 = 1
          and sc.sa_id = sa.sa_id
          and sc.char_type_cd = 'HIGH-SA '
          and sc.srch_char_val = nrm.sa_id
   )
and a.cis_division in (select /*+ cardinality 1*/ CAST(Trim(r.VL) as char(5)) cis_division from TABLE( VZ_REPORT_API.split('${PARAMETER_01}')) r)  
   #';
   l_index number;
begin
    SELECT t.idx into l_index from VZ_TASK_BASE t where t.TASK_CD = l_task_cd and t.CAT_CD = l_cat_cd;

	merge into VZ_TASK_BASE_LINE tgt
	using (
	   SELECT l_index      TASK_IDX
            , l_seq 	   SEQ
            , l_ENTITY_NM  ENTITY_NM
            , l_SHEET_NM   SHEET_NM
            , l_SHEET_COL  SHEET_COL
            , l_SQL_OP 	   SQL_OP
            , l_ENABLE 	   ENABLE
            , l_EXPORT 	   EXPORT
            , l_LINE_BUILD LINE_BUILD
            , l_DESCR 	   DESCR
            , l_SQL_HEADER SQL_HEADER
            , l_SQL_QUERY  SQL_QUERY
            , l_SQL_DATA   SQL_DATA
        FROM dual
	) src
	on (tgt.TASK_IDX = src.TASK_IDX and tgt.SEQ = src.SEQ)
	WHEN MATCHED THEN
		UPDATE set tgt.ENTITY_NM       = src.ENTITY_NM
				 , tgt.SHEET_NM        = src.SHEET_NM
                 , tgt.SHEET_COL       = src.SHEET_COL
				 , tgt.SQL_OP          = src.SQL_OP
				 , tgt.ENABLE          = src.ENABLE
				 , tgt.EXPORT          = src.EXPORT
				 , tgt.LINE_BUILD      = src.LINE_BUILD
				 , tgt.DESCR           = src.DESCR
                 , tgt.SQL_HEADER      = src.SQL_HEADER
				 , tgt.SQL_QUERY       = src.SQL_QUERY
				 , tgt.SQL_DATA        = src.SQL_DATA
        WHERE tgt.LINE_BUILD < src.LINE_BUILD
	WHEN NOT MATCHED THEN
		 INSERT( tgt.TASK_IDX, tgt.SEQ, tgt.ENTITY_NM, tgt.SHEET_NM, tgt.SHEET_COL, tgt.SQL_OP, tgt.ENABLE, tgt.EXPORT, tgt.LINE_BUILD, tgt.DESCR, tgt.SQL_HEADER, tgt.SQL_QUERY, tgt.SQL_DATA)  
		 VALUES( src.TASK_IDX, src.SEQ, src.ENTITY_NM, src.SHEET_NM, src.SHEET_COL, src.SQL_OP, src.ENABLE, src.EXPORT, src.LINE_BUILD, src.DESCR, src.SQL_HEADER, src.SQL_QUERY, src.SQL_DATA)  
	;
	commit;
end;
/

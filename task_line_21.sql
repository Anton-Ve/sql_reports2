...
declare
   ...
   l_ENTITY_NM  VARCHAR2(32) := 'acct_21';
   ...
   l_DESCR      clob := Q'##';
   l_SQL_HEADER clob := Q'##';
   l_SQL_QUERY  clob := Q'#select * from ${acct_21}#';
   l_SQL_DATA   clob := Q'#
   select
      CAST( null as char(10) ) sp_id
    , CAST( null as date ) MM  
    , CAST( null as varchar2(255) ) RS_HIST         
    , CAST( null as char(5) ) CIS_DIVISION    
    , CAST( null as number ) SAP_DPROC          
    , CAST( null as number ) SP_DPROC       
    , CAST( null as number ) SA_DPROC   
    , CAST( null as number ) CNT       

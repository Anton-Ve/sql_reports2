...
declare
   ...
   l_ENTITY_NM  VARCHAR2(32) := 'acct_04';
   l_SHEET_NM   VARCHAR2(32) := 'Количество этажей';                     --Количество этажей нецифровое знач.
   ...
   l_DESCR      clob := Q'#4#';
   l_SQL_HEADER clob := Q'##';
   l_SQL_QUERY  clob := Q'#select * from ${acct_04}#';
   l_SQL_DATA   clob := 
Q'#select 
distinct pc.prem_id
, pc.srch_char_val as FLRCOUNT
, p.cis_division
from ci_prem_char pc
left join ci_prem p on p.prem_id=pc.prem_id
where pc.char_type_cd='FLRCOUNT'

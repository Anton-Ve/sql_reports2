...
declare
   ...
   l_ENTITY_NM  VARCHAR2(32) := 'acct_15';
   ...
   l_DESCR      clob := Q'#15#';
   l_SQL_HEADER clob := Q'##';
   l_SQL_QUERY  clob := Q'#select * from ${acct_15}#';
   l_SQL_DATA   clob := 
Q'#select vz.*, p.cis_division from ${acct_14} vz
  left join ci_prem p on p.prem_id=vz.mailing_prem_id
  where 1=1 
  and not exists (select 1 from ci_stm_cnst_dtl cd join ci_stm_cnst s on s.stm_cnst_id=cd.stm_cnst_id where s.descr50=vz.descr50 and cd.end_dt is not null)

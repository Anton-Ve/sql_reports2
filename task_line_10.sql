...
declare
   ...
   l_ENTITY_NM  VARCHAR2(32) := 'acct_10';
   l_SHEET_NM   VARCHAR2(32) := 'ƒлинные адреса';   -- ƒлинные адреса 
   ...
   l_DESCR      clob := Q'#10#';
   l_SQL_HEADER clob := Q'##';
   l_SQL_QUERY  clob := Q'#select * from ${acct_10}#';
   l_SQL_DATA   clob := 
Q'#
select da.prem_id, adr.street, da.cis_division from testdm.d_address@bidm adr,testdm.d_acct@bidm da
where adr.ADR_ID = da.ADDRESS_DELIVERY_ID and length(adr.street_type||adr.street)>90 and da.D_END_DT is null

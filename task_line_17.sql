...
declare
   ...
   l_ENTITY_NM  VARCHAR2(32) := 'acct_17';
   ...
   l_DESCR      clob := Q'##';
   l_SQL_HEADER clob := Q'##';
   l_SQL_QUERY  clob := Q'#select * from ${acct_17}#';
   l_SQL_DATA   clob := Q'#
select
      CAST( null as char(10) ) sp_id
    , CAST( null as char(10) ) sa_sp_id  
    , CAST( null as char(10) ) sa_id         
    , CAST( null as varchar2(255) ) rs_cd    
    , CAST( null as date     ) start_dttm         
    , CAST( null as date     ) stop_dttm      
    , CAST( null as date     ) beg_dt  
    , CAST( null as date     ) end_dt      
    , CAST( null as date     ) effdt       
    , CAST( null as number   ) val
    , CAST( null as number   ) period_id           
from dual
where 1=0
   #';
   l_index number;
